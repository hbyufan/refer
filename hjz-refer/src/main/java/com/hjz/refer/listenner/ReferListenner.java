package com.hjz.refer.listenner;
...
import com.hjz.refer.constants.SupportUrlconstants;
import com.hjz.refer.context.ContextUtils;
import com.hjz.refer.utils.ReferCacheTool;
...
public class ReferListenner implements ApplicationListener<ApplicationContextEvent> {
	...
	public void onApplicationEvent(ApplicationContextEvent event) {
		if (event.getApplicationContext().getParent() != null) {
			...
			if (jsonbeans.size() > 0) {
				sendReferToSupportByMQ(jsonbeans);
			}
		}
	}

	/**
	 * 通过消息队列发送参照基本信息，防止support工程未启动
	 * 
	 * @param jsonbeans
	 */
	private void sendReferToSupportByMQ(JSONArray jsonbeans) {
		MqMessage message = new MqMessage();
		message.setBody(jsonbeans);
		mqSender.sendMessage(message, "MdReferbase-update");
	}
